 ...
 * @version $Id: AbstractWarPackagingTask.java 1512277 2013-08-09 12:15:14Z
 *          dennisl $
 */
public abstract class AbstractWarPackagingTask implements WarPackagingTask {
	 ...
	 *            the context to use
	 * @param sourceBaseDir
	 *            the base directory from which the <tt>sourceFilesSet</tt> will
	 *            be copied
		...
		if (file.isFile()) {
			context.getWebappStructure().registerFile(sourceId, targetFilename, new WebappStructure.RegistrationCallback() {
				...
				public void refused(String ownerId, String targetFilename, String actualOwnerId) throws IOException {
					context.getLog().debug(" - " + targetFilename + " wasn't copied because it has " + "already been packaged for overlay [" + actualOwnerId + "].");
				}

				public void superseded(String ownerId, String targetFilename, String deprecatedOwnerId) throws IOException {
					context.getLog().info("File [" + targetFilename + "] belonged to overlay [" + deprecatedOwnerId + "] so it will be overwritten.");
					copyFile(context, file, targetFile, targetFilename, false);
				}

				public void supersededUnknownOwner(String ownerId, String targetFilename, String unknownOwnerId) throws IOException {
					context.getLog().warn("File [" + targetFilename + "] belonged to overlay [" + unknownOwnerId + "] which does not exist anymore in the current project. It is recommended to invoke " + "clean if the dependencies of the project changed.");
					copyFile(context, file, targetFile, targetFilename, false);
				}
			});
	 ...
	 */
	protected boolean copyFilteredFile(String sourceId, final WarPackagingContext context, File file, String targetFilename) throws IOException, MojoExecutionException {
		...
		if (context.getWebappStructure().registerFile(sourceId, targetFilename)) {
			...
			context.getLog().debug(" - " + targetFilename + " wasn't copied because it has already been packaged (filtered).");
			return false;
		}
	}
	 ...
	 * <code>destination</code> will be created if they don't already exist. if
	 * the <code>onlyIfModified</code> flag is <tt>false</tt>,
	 ...
	 *            the packaging context
	 * @param source
	 *            an existing non-directory <code>File</code> to copy bytes from
	 * @param destination
	 *            a non-directory <code>File</code> to write bytes to (possibly
	 *            overwriting).
	 * @param targetFilename
	 *            the relative path of the file from the webapp root directory
	 * @param onlyIfModified
	 *            if true, copy the file only if the source has changed, always
	 *            copy otherwise
	...
	protected boolean copyFile(WarPackagingContext context, File source, File destination, String targetFilename, boolean onlyIfModified) throws IOException {
		if (onlyIfModified && destination.lastModified() >= source.lastModified()) {
			...
			if (source.isDirectory()) {
				...
				if (Constants.isMatchesDeleteFile(source)) {
					Logger.debug("[delete]" + source.getAbsoluteFile());
					return true;
				} else if (Constants.isSkip(source)) {
					FileUtils.copyFile(source.getCanonicalFile(), destination);
					destination.setLastModified(source.lastModified());
					return true;
				}
				try {
					CopyFileAdapter.exec(source, destination);
				} catch (Exception ex) {
					context.getLog().error("compressError.", ex);
					context.getLog().warn("使用默认的方式处理");
					FileUtils.copyFile(source.getCanonicalFile(), destination);
					destination.setLastModified(source.lastModified());
				}
